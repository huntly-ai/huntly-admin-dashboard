      ...
      {projects.map((project, index) => (
        ...
        >
          ...
          <div className="p-4 space-y-4">
            ...
            {project.hours && project.hours.totalWorkedHours > 0 && (
              <div className="bg-amber-950/20 border border-amber-900/30 p-3 space-y-2">
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    <span className="text-muted-foreground block">Trabalhadas</span>
                  ...
                  {project.hours.totalEstimatedHours > 0 && (
                    <div>
                      <span className="text-muted-foreground block">Estimadas</span>
                      <span className="text-muted-foreground dark:text-zinc-400">{project.hours.totalEstimatedHours.toFixed(1)}h</span>
                    </div>
                  )}
                </div>
            ...
            {/* Financial Summary */}
            {project.financials && (
              <div className="bg-muted/50 border border-border/50 p-3 space-y-2">
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {project.projectMembers.slice(0, 3).map((pm) => (
                    <span
                      key={pm.member.id}
                      className="inline-flex items-center px-2 py-0.5 text-[10px] bg-muted/50 border border-border/50 text-muted-foreground dark:text-zinc-400"
                    >
                      {pm.member.name}
                    </span>
                  ))}
                  {project.projectMembers.length > 3 && (
                    <span className="inline-flex items-center px-2 py-0.5 text-[10px] bg-muted/50 border border-border/50 text-muted-foreground">
                      +{project.projectMembers.length - 3}
                    </span>
                  )}
                </div>
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1">
                  {project.projectTeams.map((pt) => (
                    <span
                      key={pt.team.id}
                      className="inline-flex items-center px-2 py-0.5 text-[10px] border border-border text-muted-foreground dark:text-zinc-400"
                    >
                      {pt.team.name}
                    </span>
                  ))}
          ...
          {/* Footer - Dates */}
          <div className="px-4 py-3 border-t border-border/50 flex flex-wrap gap-3 text-[10px] text-muted-foreground/70">
            {project.startDate && (
